getActiveDocumentContext <- function() {
  editor_context <- request_client("rstudioapi/active_editor_context", args = list())
...
insertText <- function(location, text, id = NULL) {
  if (missing(text) && is.character(location) && length(location) == 1) {
    return(invisible(request_client(
      "rstudioapi/replace_text_in_current_selection",
      args = list(text = location, id = id)
    )))
  } else if (missing(location)) {
    return(invisible(request_client(
      "rstudioapi/replace_text_in_current_selection",
      args = list(text = text, id = id)
    )))
  ...
  invisible(request_client("rstudioapi/insert_or_modify_text", args = list(query = query, id = id)))
}
...
showDialog <- function(title, message, url = "") {
  message <- sprintf("%s: %s \n%s", title, message, url)
  invisible(request_client("rstudioapi/show_dialog", args = list(message = message)))
}

navigateToFile <- function(file, line = 1L, column = 1L) {
  invisible(request_client("rstudioapi/navigate_to_file", args = list(
...
setSelectionRanges <- function(ranges, id = NULL) {
  ...
  invisible(request_client(
    ...
    args = list(ranges = sess_ranges, id = id)
  ))
}
...
documentSave <- function(id = NULL) {
  invisible(request_client("rstudioapi/document_save", args = list(id = id)))
}

getActiveProject <- function() {
  path_object <- request_client("rstudioapi/get_project_path", args = list())
...
documentNew <- function(text = "", type = c("r", "rmarkdown", "sql"),
  ...
  invisible(request_client("rstudioapi/document_new", args = list(
    text = text,
    type = match.arg(type),
...
restartSession <- function(command = "", clean = FALSE) {
  invisible(notify_client("restart_r", params = list(command = command, clean = clean)))
}
...
versionInfo <- function() {
  list(
    citation = "", mode = "generic-ipc", version = numeric_version("0"),
    release_name = "generic-ipc"
  )
}

sendToConsole <- function(code, echo = TRUE, execute = TRUE, focus = TRUE, animate = FALSE) {
  if (!echo) {
    warning("rstudioapi::sendToConsole echo = FALSE is not supported in the generic IPC client.")
  }
  code_to_run <- paste0(code, collapse = "\n")
  invisible(notify_client("rstudioapi/send_to_console", params = list(
    code = code_to_run, execute = execute, focus = focus, animate = animate
  )))
}

documentClose <- function(id = NULL, save = TRUE) {
  invisible(request_client("rstudioapi/document_close", args = list(id = id, save = save)))
}

.sess_not_yet_implemented <- function(...) {
  stop("This {rstudioapi} function is not currently implemented for generic IPC.")
}
...
extract_range_text <- function(range, content_lines) {
  ...
  if (end_row <= length(content_lines)) {
    content_rows[length(content_rows)] <- substring(
      content_rows[length(content_rows)], 1, range$end["column"] - 1
    )
  }
...
normalise_pos_or_range_arg <- function(location) {
  if (rstudioapi::is.document_position(location)) {
    ...
    lapply(location, function(a_location) {
      is_pos <- rstudioapi::is.document_position(a_location)
      is_range <- rstudioapi::is.document_range(a_location)
      if (is_pos || is_range) {
        a_location
