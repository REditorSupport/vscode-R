...
function startLanguageService(): void {
  ...
  async function didOpenTextDocument(document: TextDocument) {
    ...
    if (document.uri.scheme === 'vscode-notebook-cell') {
      ...
      if (!checkClient(key)) {
        ...
        const client = await createClient(config, documentSelector,
    ...
    if (folder) {
      ...
      if (!checkClient(key)) {
        ...
        const client = await createClient(config, documentSelector, folder.uri.fsPath, folder, outputChannel);
        client.start();
        clients.set(key, client);
        initSet.delete(key);
      }
      ...
      if (document.uri.scheme === 'untitled') {
        ...
        if (!checkClient(key)) {
          ...
          const client = await createClient(config, documentSelector, os.homedir(), undefined, outputChannel);
          client.start();
          clients.set(key, client);
          initSet.delete(key);
        }
...
function stopLanguageService(): Thenable<void> {
